...
test("completes instantly and buffs fire rate inside the Deadeye window", () => {
  const weapon = deriveWeapon({ deadeye: true }, 0);
...
test("Deadeye fire-rate buff expires at its deadline", () => {
  ...
  expect(deriveWeapon({ deadeye: true }, fireRateBuffAt(buffed, buffed.buffUntil - 0.001)).fireRate)
    .toBeCloseTo(weapon.fireRate * (1 + weapon.activeBuff));
  expect(deriveWeapon({ deadeye: true }, fireRateBuffAt(buffed, buffed.buffUntil)).fireRate).toBe(weapon.fireRate);
});
