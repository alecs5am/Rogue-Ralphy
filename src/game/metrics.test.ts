...
test("reports strict rolling three-second DPS globally and per target", () => {
  ...
  metrics = recordHit(metrics, 100, 1, "dummy-1", "projectile-1");
  metrics = recordHit(metrics, 50, 2, "dummy-1", "projectile-1");
...
test("counts ricochet impacts once for projectile accuracy and waits to resolve misses", () => {
  ...
  metrics = recordHit(metrics, 20, 1, "dummy-1", "projectile-1");
  metrics = recordHit(metrics, 18, 2, "dummy-2", "projectile-1");
  ...
  metrics = recordProjectileOutcome(metrics, "projectile-2", false);
  expect(summarizeMetrics(metrics, 2)).toMatchObject({ successfulProjectiles: 1, misses: 1, accuracy: 0.5 });
  expect(recordProjectileOutcome(metrics, "projectile-2", false)).toEqual(metrics);
